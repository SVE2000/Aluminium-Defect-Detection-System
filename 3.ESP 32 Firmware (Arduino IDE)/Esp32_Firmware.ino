...
volatile bool conveyorDirection = HIGH;  // HIGH = clockwise, LOW = counterclockwise
...
void setup() {
  ...
  Serial.println("\n=== READY ===");
  Serial.println("Test: http://" + WiFi.localIP().toString() + "/continuous?speed=500&dir=cw");
}
...
void handleContinuous() {
  ...
  if (server.hasArg("speed")) {
    ...
    // Get direction parameter (default to clockwise if not specified)
    bool direction = HIGH; // Default clockwise
    if (server.hasArg("dir")) {
      String dirArg = server.arg("dir");
      dirArg.toLowerCase();
      if (dirArg == "ccw" || dirArg == "counterclockwise" || dirArg == "reverse") {
        direction = LOW;
      }
    }
    ...
    if (speed >= 50 && speed <= 5000) {
      ...
      digitalWrite(CONVEYOR_EN_PIN, LOW);
      delay(10);
      digitalWrite(CONVEYOR_DIR_PIN, direction);
      delay(5);
      ...
      response += ",\"direction\":\"";
      response += (direction == HIGH) ? "cw" : "ccw";
...
void handleStatus() {
  ...
  status += ",\"conveyor_direction\":\"";
  status += (conveyorDirection == HIGH) ? "cw" : "ccw";
...
void handleRoot() {
  ...
  html += ".purple{background:#6f42c1;}";
  html += "h1{color:#333;}";
  html += "h3{color:#666;margin-top:10px;margin-bottom:5px;}";
  html += ".container{background:white;padding:20px;border-radius:10px;}";
  html += ".section{margin:20px 0;padding:15px;background:#f8f9fa;border-radius:8px;}";
  html += "</style></head><body><div class='container'>";
  html += "<h1>Stepper Motor Control</h1>";
  html += "<p><strong>IP:</strong> " + WiFi.localIP().toString() + "</p>";
  
  html += "<div class='section'>";
  html += "<h2>Conveyor - Clockwise (CW)</h2>";
  html += "<h3>Speed Control</h3>";
  html += "<a href='/continuous?speed=100&dir=cw' class='btn green'>Very Fast (100us)</a>";
  html += "<a href='/continuous?speed=200&dir=cw' class='btn green'>Fast (200us)</a>";
  html += "<a href='/continuous?speed=500&dir=cw' class='btn green'>Medium (500us)</a>";
  html += "<a href='/continuous?speed=1000&dir=cw' class='btn green'>Slow (1000us)</a>";
  html += "</div>";
  
  html += "<div class='section'>";
  html += "<h2>Conveyor - Counter-Clockwise (CCW)</h2>";
  html += "<h3>Speed Control</h3>";
  html += "<a href='/continuous?speed=100&dir=ccw' class='btn purple'>Very Fast (100us)</a>";
  html += "<a href='/continuous?speed=200&dir=ccw' class='btn purple'>Fast (200us)</a>";
  html += "<a href='/continuous?speed=500&dir=ccw' class='btn purple'>Medium (500us)</a>";
  html += "<a href='/continuous?speed=1000&dir=ccw' class='btn purple'>Slow (1000us)</a>";
  html += "</div>";
  ...
  html += "<li>Wrong direction? Swap motor wiring or use opposite direction</li>";
